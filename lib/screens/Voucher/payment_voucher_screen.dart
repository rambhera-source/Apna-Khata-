...
class _PaymentVoucherScreenState extends State<PaymentVoucherScreen> {
  ...
  void _generateVoucherNumber() {
    _voucherNumber = 'PMT-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    setState(() {});
  }
  ...
  void _addCreditRowWithRemaining() {
    double diff = _totalDebit - _totalCredit;
    if (diff > 0) {
      setState(() {
        _creditRows.add({
          'accountController': TextEditingController(text: 'Cash-in-Hand'),
  ...
  Future<void> _savePaymentVoucher() async {
    ...
    if ((drTotal - crTotal).abs() > 0.01) {
      // Auto-balance remaining amount into a new credit row if mismatched
      _addCreditRowWithRemaining();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Remaining amount automatically adjusted in Credit row.'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    String sourceSummary = _creditRows.first['accountController'].text.trim();
    if (sourceSummary.isEmpty) sourceSummary = 'Cash-in-Hand';

    String notes = _notesController.text.trim();

    await DatabaseHelper.isar.writeTxn(() async {
      // Save Debit transactions (Positive amount to increase/debit party balance)
      for (var r in _debitRows) {
      ...
      // Save Credit transactions (Negative amount to reduce Cash/Bank/Source balance)
      for (var r in _creditRows) {
        String accName = r['accountController'].text.trim();
        double amt = double.tryParse(r['amountController'].text) ?? 0.0;
        if (accName.isNotEmpty && amt > 0) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Payment Voucher ($_voucherNumber) has been saved successfully.', style: const TextStyle(fontSize: 13)),
        actions: [
          ElevatedButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Payment Voucher Entry'),
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              // Credit Section (Cr) - Source (Cash/Bank)
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Credit Accounts (Cr - Source):', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.green)),
                  TextButton.icon(
                    style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(50, 30)),
                    ...
                    onPressed: _addCreditRowWithRemaining,
                  ),
                ],
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text('Save Payment Voucher', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
